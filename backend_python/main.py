...
def verify_qr_and_data(data: ValidationRequest):
    ...
    original_img, ocr_img = fetch_image_from_url(data.image_url)
    ...
    cv2.imwrite("debug_certificate.jpg", original_img)
    ...
    for scale in [1, 2, 3, 4]:
        ...
        qr_data, bbox, _ = detector.detectAndDecode(resized)

        if qr_data:
            qr_url = qr_data.strip()
            print(f"✅ QR Found at scale {scale}: {qr_url}")
            break

    # Fallback: OCR URL detection
    if not qr_url:
        found_urls = url_extractor.find_urls(
            cert_text_string.replace("\n", " ")
        )
        ...
        if len(found_urls) > 0:
            ...
            print(f"✅ Found URL via OCR: {qr_url}")
    ...
    if qr_url:
        ...
        try:
            with sync_playwright() as p:
                ...
                page.goto(qr_url, wait_until="networkidle", timeout=150000)
